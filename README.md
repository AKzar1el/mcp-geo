...
Want to reproduce the same evidence-first structure with your own data? Use the [reusable AI Visibility Audit report prompt](docs/ai-visibility-audit-report-prompt.md).
...
- **Complete Claude Desktop metadata:** the MCPB manifest now declares all fixed local tools, including `track_brand`, `list_brands`, `list_prompts`, and `generate_prompts`, matching the actual stdio server exposed after install.
...
- **Local brand-management tools** (CLI only): `track_brand`, `list_brands`, `list_prompts`, `generate_prompts`. Workers deployments keep these behind the `X-Seed-Secret`-gated `/admin/*` routes.
...
- Local brand-management tools: `track_brand`, `list_brands`, `list_prompts`, `generate_prompts`.
