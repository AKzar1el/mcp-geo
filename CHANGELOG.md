...
- Authenticated `/admin/run-live` scans can now set `wait_for_completion: true` so manual/operator refreshes stay attached to the HTTP request until per-engine service-binding work finishes instead of depending on Cloudflare's 30-second post-response `waitUntil()` window.
